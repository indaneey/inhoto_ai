...
async function fetchStickers(categoryId, limit = 15) {
    try {
        const response = await fetch(`${API_BASE}/api/stickers?category=${categoryId}&page=1&limit=${limit}`);
...
async function fetchComponents(categoryId, limit = 10) {
    try {
        const response = await fetch(`${API_BASE}/api/elements?category=${categoryId}&page=1&limit=${limit}`);
        const data = await response.json();
...
function scoreByTags(assetTags, desiredTags) {
    if (!assetTags || !Array.isArray(assetTags))
        return 0;
